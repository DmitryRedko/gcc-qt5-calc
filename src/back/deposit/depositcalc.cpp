...
{
    ui->setupUi(this);
    connect(ui->addRefill, &QPushButton::clicked, this, &DepositCalc::onAddRefillClicked);
    connect(ui->addPayoff, &QPushButton::clicked, this, &DepositCalc::onAddPayoffClicked);
    connect(ui->calculate, &QPushButton::clicked, this, &DepositCalc::onCalculateClicked);
    ui->startDate->setDate(QDate::currentDate());
    ui->startDate->setMinimumDate(QDate::currentDate());

    QDoubleValidator* validator_sum =  new QDoubleValidator( 0, 10000000000000.0, 5 );
...
void DepositCalc::onAddRefillClicked() {
    ...
    // Получаем текущий макет QFrame (ui->refillFrame) и устанавливаем в него новый макет
    ...
    // Подключаем слот для удаления строки при нажатии на кнопку "Удалить"
        connect(delete_button, &QPushButton::clicked, this, [=]() {
            v_layout->removeItem(h_layout); // Удаляем макет строки из вертикального макета
...
void DepositCalc::onAddPayoffClicked() {
    // Создаем элементы интерфейса и компоновку элементов (аналогично предыдущему коду)
    QLineEdit *line = new QLineEdit(this);
    QDateEdit *date_edit = new QDateEdit(this);
    date_edit->setMinimumDate((QDate::currentDate()).addDays(1));
    ...
    // Подключаем слот для удаления строки погашения при нажатии на кнопку "Удалить"
    connect(delete_button, &QPushButton::clicked, this, [=]() {
        v_layout->removeItem(h_layout); // Удаляем макет строки из вертикального макета
        delete line; // Удаляем элементы строки
        delete date_edit;
...
void DepositCalc::onCalculateClicked() {
    ...
    QString result = calculateInterestAndTaxes( deposit_num, interest_num, tax_num, partial_add_mass, in_dates, partial_payoff_mass, in_dates_del, final_date);

    ui->result->setText(result);
}

void DepositCalc::extractRefillInfo(QList<double>& partial_add_mass, QList<QDate>& in_dates) {
...
void DepositCalc::extractPayoffInfo(QList<double>& partial_payoff_mass, QList<QDate>& in_dates_del) {
    ...
    for (int i = 0; i < lines_del.count(); i += 2) {
        ...
        in_dates_del.append(QDate::fromString(lines_del[i + 1]->text(), "dd.MM.yyyy"));
    }
}
...
QString DepositCalc::calculateInterestAndTaxes(double deposit_num, double interest_num, double tax_num,
    ...
    while (current_date <= final_date) {
        ...
        if (QString::compare(ui->payTime->currentText(), "день") == 0) {
            ...
            result = result + current_date.toString("dd.MM.yyyy") + "->" + QString::number(earned, 'f', 2) + "\n";
            earned = 0;
            taxed = 0;
        } else if (QString::compare(ui->payTime->currentText(), "месяц") == 0) {
            if (current_date == started_date.addMonths(1) || current_date == final_date) {
            ...
            if (current_date == started_date.addYears(1) || current_date == final_date) {
                started_date = current_date;
    ...
    result = result + "Баланс на конец срока:" + QString::number(deposit_num, 'f', 2) + "\nПроценты:" + QString::number(overal, 'f', 2);
