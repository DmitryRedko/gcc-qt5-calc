...
void DrawGraphics::onDrawClicked()
{
    ...
    if(!QuationString.isEmpty() && (Xmin<Xmax) && (Ymin<Ymax) &&(!ui->x_min->text().isEmpty()) &&(!ui->x_max->text().isEmpty()) &&(!ui->y_min->text().isEmpty()) &&(!ui->y_max->text().isEmpty()))
    {
        ui->widget->yAxis->setRange(Ymin,Ymax);
        ui->widget->xAxis->setRange(Xmin,Xmax);
        ui->widget->addGraph();
        ui->widget->replot();
        QByteArray byteArray = QuationString.toLatin1(); // Преобразуем QString в QByteArray
        ...
        Token *tokens = tokenize(exprCStr); // Теперь передаем C-style строку
           ...
           for(int i = 0; i < 1000;i++){
               ...
               if(flag_calc ==0 && !std::isnan(res_number) && !std::isinf(res_number)){
                   x.push_back(X);
