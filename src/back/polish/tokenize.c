...
Token parse_token(const char *expr, int *pos) {
    ...
    int flag_check = 0;
    // Если достигнут конец строки, то возвращаем лексему TOK_END
    if (expr[*pos] == '\0') {
        token.type = TOK_END;
        return token;
    }
...
void if_operand_second_priority(const char *expr, int *flag_check, Token *token, int *pos) {
    if (expr[*pos] == '+' && *flag_check == 0) {

        char prev_char = expr[*pos - 1];
        // Распознаем унарный плюс
        if (*pos == 0 || prev_char == '(' || prev_char == '+' || prev_char == '-' || prev_char == '*' ||
            prev_char == '/') {
...
void if_operand_third_priority(const char *expr, int *flag_check, Token *token, int *pos) {
    if (expr[*pos] == '*' && *flag_check == 0) {
...
void if_function(const char *expr, int *flag_check, Token *token, int *pos) {
    ...
    if (strncmp(expr + *pos, "tg", 2) == 0 && *flag_check == 0) {
        (*token).type = TOK_FUNC_TG;
        (*token).flag_function = 4;
        *flag_check = 1;
        (*pos) += 2;
    }
    if (strncmp(expr + *pos, "atg", 3) == 0 && *flag_check == 0) {
