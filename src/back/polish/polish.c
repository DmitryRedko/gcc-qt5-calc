...
Token *convert_to_polish(Token *mass) {
    ...
    for (int i = 0; mass[i].type != TOK_END; i++) deikstra(mass, i, stack_res, stack_operands);
    ...
    for (int i = 0; i < stack_res->top + 1; i++) res_mass[i] = stack_res->items[i];
...
void deikstra(Token *mass, int i, Stack *stack_res, Stack *stack_operands) {
    // printf("STACK_RES:\n");
    // print_stack(stack_res);
    // printf("STACK_OPERANDS:\n");
    ...
    if (mass[i].flag_function > 1 && mass[i].flag_function < 5) {
        if (is_empty(stack_operands) || stack_operands->items[stack_operands->top].flag_function < mass[i].flag_function )
            push(stack_operands, mass[i]);
        else {
            while (stack_operands->items[stack_operands->top].flag_function >= mass[i].flag_function ) {
                if(is_empty(stack_operands))
                    break;
                // if(kount == 100) break;
                // printf("\n%d %d\n",stack_operands->items[stack_operands->top].flag_function, mass[i].flag_function);
                // printf("STACK_RES:\n");
                // print_stack(stack_res);
                // printf("STACK_OPERANDS:\n");
                // print_stack(stack_operands);
...
void check_for_function(Token *mass, int i, Stack *stack) {
    if (mass[i].flag_function == 4) {
        ...
        if (mass[i].type == TOK_FUNC_TG) {
            if (fabs(x.value - M_PI / 2) < EPS || fabs(x.value - M_PI * 3 / 2) < EPS ||
                fabs(x.value - M_PI * 5 / 2) < EPS || fabs(x.value - M_PI * 7 / 2) < EPS)
