    ...
    ui(new Ui::CreditCalc)
{
    ...
    connect(ui->calculate, &QPushButton::clicked, this, &CreditCalc::onCalculateClicked);
...
void CreditCalc::onCalculateClicked(){
        ui->paysheet->setText("");
        if(!ui->sumInput->text().isEmpty() && !ui->rateInput->text().isEmpty() && !ui->timeInput->text().isEmpty() && (ui->annuity->isChecked() || ui->differencial->isChecked())){
            ...
            if(ui->annuity->isChecked()){
                ...
                double month_pay = 0;
                month_pay = sum*(rate/1200.0+(rate/1200.0)/(pow(rate_plus_one,time)-1.0));
                ui->monthPayOutput->setText(QString::number(month_pay, 'f', 2));
                ui->fullPayOutput->setText(QString::number(month_pay*time, 'f', 2));
                ui->overPayedOutput->setText(QString::number(month_pay*time-sum, 'f', 2));
                ui->paysheet->setText("№\tВыплата\tОстаток долга\n");
                double debt = sum;
                 for(int i=0; i<time; i++){
                     debt=debt*rate_plus_one-month_pay;
                     if(debt<0) debt = 0;
                     ui->paysheet->setText(ui->paysheet->toPlainText()+QString::number(i+1) + "\t" + QString::number(month_pay, 'f', 2) + "\t" + QString::number(debt, 'f', 2) + "\n");
                 }
            }
            ...
            if(ui->differencial->isChecked()){
                ...
                for (int i = 0; i < time; ++i) {
                   ...
                   ui->paysheet->setText(ui->paysheet->toPlainText()+QString::number(i+1) + "\t" + QString::number(monthly_payment, 'f', 2) + "\t" + QString::number(remain_sum, 'f', 2) + "\n");
               }
                min_payment = monthly_payment;
                ui->monthPayOutput->setText(QString::number(min_payment, 'f', 2) + " - " + QString::number(max_payment, 'f', 2));
