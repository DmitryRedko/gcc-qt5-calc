    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->numbs, QOverload<QAbstractButton *>::of(&QButtonGroup::buttonClicked), this, &MainWindow::handleButtonClicked);
    connect(ui->functions, QOverload<QAbstractButton *>::of(&QButtonGroup::buttonClicked), this, &MainWindow::handleButtonClicked);
    connect(ui->operands, QOverload<QAbstractButton *>::of(&QButtonGroup::buttonClicked), this, &MainWindow::handleButtonClicked);
    connect(ui->brackets, QOverload<QAbstractButton *>::of(&QButtonGroup::buttonClicked), this, &MainWindow::handleButtonClicked);
    connect(ui->equal, &QPushButton::clicked, this, &MainWindow::onEqualClicked);

    connect(ui->floatDot, &QPushButton::clicked, this, [this](bool) {
        handleDotClicked(ui->floatDot);
    });
    ...
    connect(ui->credit, &QPushButton::clicked, this, &MainWindow::onCreditClicked);
    connect(ui->deposit, &QPushButton::clicked, this, &MainWindow::onDepositClicked);
...
void MainWindow::onDrawClicked(){
    ...
    connect(this,&MainWindow::sendQuationString,&window,&DrawGraphics::getQuationString);
...
void MainWindow::enableButtons(QAbstractButton *button) {
    ...
    if(countRightBrackets<countLeftBrackets && ui->lbracket != button && !ui->functions->buttons().contains(button) && !ui->operands->buttons().contains(button) && ui->floatDot != button)
...
void MainWindow::onEqualClicked() {
        ...
        Token *tokens = tokenize(exprCStr); // Теперь передаем C-style строку
           ...
           if (flag == 0) {
               ...
               double res_number = 0;
               if (ui->xInput->text().isEmpty() && ui->result->text().contains(ui->x->text())) {
                 QMessageBox::critical(this, "Ошибка", "Требуется указать значение х.");
               } else {
                 res_number = calculate(res, ui->xInput->text().toDouble(), &flag_calc);
               }
               if(flag_calc == 0) {
                    ui->result->setText(QString::number(res_number)); // Преобразование double в строку
               }
               else{
